...
def run_train_lstm_for_lemon():
    ...
    train_x = data_x.reshape((len(data_x), inp_dim))
    train_y = data_y.reshape((len(data_y), out_dim))
    ...
    for b in range(train_size // batch_size):
        batch_var_x_tmp, batch_var_y_tmp = [], []
        for j in range(batch_size):
            idx = b * batch_size + j
            batch_var_x_tmp.append(train_x[idx:idx + 12])
            batch_var_y_tmp.append(train_y[idx + 12])
        batch_var_x.append(batch_var_x_tmp)
        batch_var_y.append(batch_var_y_tmp)
    #
    # with torch.no_grad():
    #     weights = np.tanh(np.arange(len(train_y)) * (np.e / len(train_y)))
    #     weights = torch.tensor(weights, dtype=torch.float32, device=device)
    ...
    for e in range(184):
        for b in range(train_size // batch_size):
            x = torch.tensor(batch_var_x[b], dtype=torch.float32, device=device)
            y = torch.tensor(batch_var_y[b], dtype=torch.float32, device=device)
            out = net(x)
        ...
        print(f'Epoch: {e}/{384}, Loss: {loss.item()}')
    torch.save(net.state_dict(), '{}/net.pth'.format(mod_dir))
    print("Save in:", '{}/net.pth'.format(mod_dir))
    ...
    test_x = data_x.reshape((len(data_x), inp_dim)).copy()
    test_x[train_size + 12:] = 0
    test_x = torch.tensor(test_x, dtype=torch.float32, device=device)

    '''simple way but no elegant'''
    for b in range(train_size, len(data) - 12):
        test_y = net.output(test_x[b:b + 12])
        test_x[b + 12] = test_y

    '''elegant way but slightly complicated'''
    # eval_size = 1
    # zero_ten = torch.zeros((mid_layers, eval_size, mid_dim), dtype=torch.float32, device=device)
    # test_y, hc = net.output_y_hc(test_x[train_size:train_size + 12], (zero_ten, zero_ten))
    # test_x[train_size + 1, 0, 0] = test_y[-1]
    # for b in range(train_size + 1, len(data) - 2):
    #     test_y, hc = net.output_y_hc(test_x[b:b + 1], hc)
    #     test_x[b + 1, 0, 0] = test_y[-1]
    ...
    plt.plot(pred_y, 'r', label='pred')
    plt.plot(data_y, 'b', label='real', alpha=0.3)
    plt.plot([train_size + 12, train_size + 12], [-1, 2], color='k', label='train | pred')
    plt.legend(loc='best')
    plt.savefig('lstm_reg.png')
    plt.pause(4)
...
class RegLSTM(nn.Module):
    def __init__(self, inp_dim, out_dim, mid_dim, mid_layers):
        ...
        self.rnn = nn.LSTM(inp_dim, mid_dim, mid_layers, batch_first=True)  # rnn
        self.reg = nn.Sequential(
            nn.Linear(mid_dim * 12, mid_dim),
            nn.Tanh(),
            nn.Linear(mid_dim, out_dim),
        )  # regression

    def forward(self, x):
        x = self.rnn(x)[0]  # y, (h, c) = self.rnn(x)
    ...
    def output_y_hc(self, x, hc):
        x, hc = self.rnn(x, hc)  # y, (h, c) = self.rnn(x)

...
class ENSOConvLSTM(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, output_dim, kernel_size, num_layers,
                 batch_first=False, bias=True, return_all_layers=False):
        super(ENSOConvLSTM, self).__init__()
    ...
    def forward(self, input_tensor):
        b, t, c, h, w = input_tensor.size()  # 自动获取 b,t,c,h,w信息
        layer_output_list, last_state_list = self.convLSTM(input_tensor)
        last_layer_output = layer_output_list[-1]  # 400, 3, 16, 24, 72
        last_layer_last_h, last_layer_last_c = last_state_list[-1]  # 400, 16, 24, 72

        linear1 = nn.Linear(t * 16 * h * w + 16 * h * w + 16 * h * w, t * h * w)
        linear2 = nn.Linear(t * h * w, self.output_dim)

        x = torch.cat((torch.cat((
            last_layer_output.reshape(-1, t * 16 * h * w),
            last_layer_last_h.reshape(-1, 16 * h * w)), 1),
            last_layer_last_c.reshape(-1, 16 * h * w)), 1)
        x = linear1(x)
        x = linear2(x)
...
if __name__ == "__main__":
    ...
    model = ENSOConvLSTM(input_dim=2,
                         hidden_dim=[8, 16, 16],
                         output_dim=23,
                         kernel_size=[(3, 3), (5, 5), (7, 7)],
                         num_layers=3,
                         ...
                         bias=True,
                         return_all_layers=True).to(device)
    data = torch.randn((400, 3, 2, 24, 72)).to(device)
    output = model(data)
    print(output)
    print(output.size())
